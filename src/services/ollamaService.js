...
/**
 * Améliore le nom d'un acteur pour le format DBpedia
 * @param {string} actorName - Nom brut saisi par l'utilisateur
 * @returns {Promise<string>} - Nom formaté pour DBpedia
 */
        ...
        const response = await fetch(OLLAMA_API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'Tu es un expert en DBpedia. Quand on te donne un nom d\'acteur, tu retournes UNIQUEMENT le nom exact tel qu\'il apparaît dans DBpedia (format: "Prenom Nom" en anglais). Pas d\'explication, juste le nom.'
                    },
                    {
                        role: 'user',
                        content: `Quel est le nom exact de cet acteur dans DBpedia: "${actorName}"`
                    }
                ],
                temperature: 0.1
            })
